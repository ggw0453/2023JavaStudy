...
/*
시나리오] 여러개의 숫자를 입력받은후 최대값을 구하는 메소드를 정의하시오. 단, 숫자의
갰수는 매개변수로 전달받고 숫자의 갯수만큼 Scanner클래스를 통해 입력받늗다.
	...
	static int returnMaxNumber(int numberCnt) {
		...
		for(int i=1 ; i<=numberCnt ; i++) {
			...
			if(i==1) {
				...
				위한 기준값으로 설정한다. 즉 비교의 대상이 없으므로 무조건 최대값이 된다. */
				maxVal = inputNum;
			}
			else {
		...
		/* 즉, 새로운 입력값을 기존값보다 크다면 지속적으로 교체하여 가장 큰 값을
		 호출한 지점으로 반환한다. */
		return maxVal;
	}
