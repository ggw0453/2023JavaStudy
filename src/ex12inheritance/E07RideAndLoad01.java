...
class C extends B {
	...
	/*
	 따라서 c클래스로 인스턴스를 생성하면 오버라이딩 된 메서드는 모두 가려지게 되어 c클래스에
	 정의된 메서드 1개만 남게된다. 오버로딩 된 메서드는 서로 다르므로 3개 모두 남게된다.
	 */
}
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/* 기본자료형의 경우에는 작은타입이 큰타입에 할당되는 것은 자동형 변환
		이 되기 때문에 아래와 같이 작성할 수 있다.
		클래스도 이와 마찬가지로 부모타입(큰타입)으로 자식인스턴스(작은타입)를 참조할 수 있다. */
		int a = 10;
		double b = 3.14;
		double c = 20;//int가 double로 자동형 변환 되어 할당된다.
		//int d = 3.14 //double은 int에 저장할 수 없다. 왜냐면 실수는 정수보다
		//큰 타입이기 때문이다.
		//위의 문장이 가능하게 되려면 강제형변환를 하면 된다.
		int d = (int) 3.14;
		
		/*
		 참조변수에 상관없이 항상 최하위에 오버라이딩된 메서드가 호출된다. 즉 아래 부분은
		 모두 C클래스에 정의된 rideMethod()가 호출된다. 오버라이딩은 참조변수의 영향을
		 받지 않는다.
		 */
		...
		/* B타입으로는 접근범위가 B클래스까지로 제한되므로 C에 정의된 메서드는
		호출할 수 없다. */
		ref2.loadMethod();
		ref2.loadMethod(1);
		//ref2.loadMethod(1.1);
		
		/* A타입으로는 접근범위가 A클래스까지로 제한되므로 B와 C에 정의된 메서드는
		호출할 수 없다. */
		ref1.loadMethod();
		//ref1.loadMethod(1);
		//ref1.loadMethod(1.1);
		...
		변경하는 것이다. 이는 클래스가 상속관계에 있기 때문에 가능하고, 방식은 기본자료형과
		동일하게 소괄호를 이용하면 된다. */
		B refId2 = (B)refId1;
		C refId3 = (C)refId1;
	}
