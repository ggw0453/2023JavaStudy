...
//부모클래스
class Account {
	/* 멤버변수가 상속관계에 있다 하더라도 private으로 선언되면 클래스내부에서만 접근할 수 있다.
	즉 상속받은 하위클래스에서는 직접 접근이 불가능하하다
	 */
	private int money;
	
	/* 이와같이 개발자가 생성자를 직접 정의하면 디폴트 생성자는 추가되지 않는다. 디폴트
	생성자는 개발자가 생성자를 정의하지 않았을때만 컴파일러에 의해 자동으로 추가된다. */
	...
	/* 현재는 부모와 자식 클래스가 동일한 패키지에 정의되어 있으므로 접근 지정자가 default
...
class SavingAccount extends Account{
	...
	/* 인자생성자에서 부모의 생성자를 호출하기 위한 super(xx)를 사용한다. 이 때
	매개변수가 하나인 부모클래스의 생성자가 호출된다. 현재 부모클래스에는 매개변수가 하나인
	생성자가 유일하므로 만약 아래와 같이 호출하지 않으면 에러가 발생한다. */
	...
	직접 접근할 수 없다. 보이지 않으므로 에러가 발생한다. 따라서 부모클래스에 protected로
	선언된 메서드를 통해 입금처리를 해야한다. */
