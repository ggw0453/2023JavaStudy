...
/*
친구를 추사오하한 최상위 클래스로 해당 프로그램에서는 Friend 클래스로 인스턴스 생성은
하지 않는다. 단지 기본정보를ㄹ 저장하고 상속을 목적으로 정의된 클래스이다.
...
public class E10MyFriendInfoBook{ 
	...
	/* 메뉴를 출력하는 용도의 메서드로 매개변수, 반환타입이 없는 형태로 정의한다. main메서드에서 직접
	호출하기 위해 static으로 선언되어 있다. */
	...
	흐름에 대해서만 기술하는것이 좋다. 따라서 선택한 메뉴에 따라 핸들러 클래스의 메서드만 호출하는
	형태로 구현되어있다. 
	 */
	public static void main(String[] args) {
		...
		/*
		무한루프 조건으로 사용자가 원할때 종료할 수 있는 구조를 만들어준다. break문은
		반복문을 탈출시키는 기능이 있으므로 이와같이 무한루프에서 자주 사용한다.
		for(;;) 문을 토해 무한루프를 구현할 수 있으나 반복의 횟수가 명확하지 않은 경우에는
		while문을 사용한다.
		 */
		while(true) {
			//1.메뉴를 출력한다.
...
class FriendInfoHandler {
	...
	/* 배열에 저장된 연락처 정보를 카운트하기 위한 변수로, 추가할 때 마다 
	++ 연산자로 1씩 증가한다. */
	...
	public FriendInfoHandler(int num) {
		...
		//배열의 인스턴스는 0부터 시작이므로 이와같이 초기화한다.
		numOfFriends = 0;
	}
	...
	public void addFriend(int choice) {
		...
		if(choice==1) {
			...
			0이므로, 0번 인덱스에 추가된 뒤 1증가한다. 이를 위해 "후위증가"하고 있다. */
	...
	/*
 	...
 	2.이때 인스턴스를 Friend로 자동형변환(업캐스팅)되어 저장된다.
	...
	public void showSimpleData() {
		...
		정보가 저장되는 배열은 Friend 타입이므로 자식인스턴스를 저장하면 자동으로
		형변환(업캐스팅)이 된다. 이 때 자식의 정보를 출력하는 메서드를 바로 호출할
		수 없으므로 아래와 같이 일일이 확인한 후 강제형변환(다운캐스팅) 해야한다.
		...
		*/
		...
		/* 오버라이딩을 이용하면 상속의 구조가 복잡해지더라도 아래와 같이 한줄이면
		정보를 출력할 수 있다. */
		for(int i=0 ; i<numOfFriends ; i++) {
	...
	public void searchInfo() {
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			/* 배열의 각 인덱스에 저장된 인스턴스의 참조값을 통해 맴버변수에 접근한다.
	...
	public void deleteInfo() {
		...
		/* 배열의 원소 중 삭제된 원소의 인덱스값을 저장하기 위한 변수로 삭제한
		후 빈자리를 채워넣을 때 사용한다. 인덱스는 0부터 시작이므로 -1을 초기값으로
		설정한다. */
		int deleteIndex = -1;
		...
		if(deleteIndex==-1) {
			...
			/*
			인스턴스 베열에서 원소를 삭제한 후 바로 뒤에 있는 원소를 앞으로
			하나씩 복사한다. 만약 이 부분이 처리되지 않는다면 차후 검색이나
			삭제시 NullPointerException이 발생할 수 있다. 
			 */
