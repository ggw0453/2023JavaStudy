...
메서드를 제정의 하는것을 말한다. 오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이
완전히 똑같은 형태로 정의하는것이다.
하위클래스에서 오버라이딩한 메서드는 상위클래스의 메서드를 가리게 되므로 항상 하위 클래스에서 오버라이딩한
메서드가 호출된다.
 */
...
public class E06Overriding {
	public static void main(String[] args) {
		...
		/* 자식 인스턴스를 생성했으므로 부모의 메머와 자식의 멤버 모두 접근할 수 있다. */
		baseSpeaker.setVolumn(10);
		baseSpeaker.setBase(20);
		/* 오버라이딩 된 메서드를 호출하면 자식쪽의 showState()가 호출된다.
		부모쪽의 메서드는 자식에 의해 가려지게된다. 윈도우 탐색기에서 같은 이름의 파일을
		붙여넣기 할 때 덮어쓸지 물어보는 것과 동일하다 생각하면 된다. */
		...
		/* 단, 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 있는 범위가 부모인스턴스
		까지 제한되므로, 자식쪽에 정의된 setBase()는 호출할 수 없다. */
