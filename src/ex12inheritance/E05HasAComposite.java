...
: X has a Y => X가 Y를 소유(포함)하다. 여기서는 권총을 소유한 경찰을 포현한다.

Has-A와 같이 소유의 관계를 상속으로 표현하면 클래스간의 강한 연결고리가 형성되어 권총이 없는
경찰을 표현하기가 힘들어진다. 따라서 이 경우에는 구성(Composition)관계로 표현하는것이 적합하다.
...
class Police {
	...
	public Police(int bNum, int hCuff) {
		...
		생성된 인스턴스가 아예 없으므로 소유하지 않은것을 표현할 수 있다.
		 */
		if(bNum<=0) {
			gun = null;
		}
