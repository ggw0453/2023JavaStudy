...
class MyClass {
	...
	재정의한다. 인스턴스가 가진 멤버변수의 값에 대한 비교를 통해 동일여부를 판단한다.
	equals() 메서드의 매개변수 타입은 모든 인스턴스를 대상으로 하므로 Object 타입으로
	정의되어있다
	 */
	@Override
	public boolean equals(Object obj) {
		...
		if(obj instanceof MyClass) {
			/*
			매개변수가 MyClass 타입이 맞다면 비교를 위해 다운캐스팅한다. 그렇지
			않으면 부모타입으로는 자식 멤버에 접근할 수 없으므로 멤버변수 data에
			대한 비교가 불가능하다.
			 */
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		만약 오버라이딩 해놓은 equals()를 삭재한 후 비교하면 다르다는
		결과가 나오게 된다. 즉 기본클래스일때만 오버라이딩 없이 비교가 가능하다.
		 */
		System.out.println("[두 객체를 equals()메소드로 비교]");
		System.out.println(mc1.equals(mc2) ? "같다" : "다르다");
		
		/*
		print()를 통해 출력하면 자동으로 toString()을 호출하므로 아래 출력
		결과는 동일하다.
		 */
