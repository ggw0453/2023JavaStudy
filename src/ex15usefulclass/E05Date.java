...
public class E05Date {
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			따라서 각 날짜의 서식을 명확히 지정해야만 날짜를 인식하여 연산을 할 수 있다.
			...
			/*
			getTime()으로 얻어오는 시간은 13자리의 정수값인데 10자리는 순수한 초(Sec)이고,
			11~13자리는 밀리세컨즈를 의미한다. 즉 100으로 나눠서 뒤의 3자리를 제거하면 순수한
			초가 반환된다. 또한 하루는 60*60*24로 86400초 이므로 아래와 같이 남은 날짜를
			계산할 수 있다.
			 */
