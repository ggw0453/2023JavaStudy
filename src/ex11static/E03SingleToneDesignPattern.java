...
/*
 싱글톤 디자인 패턴
 : 클래스를 설계하는 디자인패턴의 하나로써 하나의 인스턴스 즉 하나의 메모리를 할당한
 후 이를 프로그램 전체에서 공유해서 사용하고자 할 때 쓰는 패턴이다. 
...
class NoSingleTone {
	...
	/* 생성자는 대부분 public으로 선언한다. 만약 private으로 선언하면
	 클래스외부 혹은 다른 패키지에서는 접근할 수 없기 때문이다. */
	public NoSingleTone() {}
}
//싱글턴 패턴이 적용된 클래스 정의
class SingleTone {
	//일반적인 멤버변수 선언
	int shareVar;
	/* 정의한 클래스와 동일한 타입으로 선언된 정적 멤버변수로 JVM(자바 가상머신)에 의해
	프로그램이 시작될 때 미리 Method영역에 로드되어 사용할 준비를 마치게 된다. */
