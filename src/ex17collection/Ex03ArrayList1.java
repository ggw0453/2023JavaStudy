...
-데이터의 중복 저장이 허용된다
-데이터의 저장 순서를 보장한다.
-Array라는 이름처럼 "배열"특성을 가지고 있어 index를 통한 저장 및 접근이 가능하다.
 */
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		객체저장 : add()
			...
			add(인덱스, 인스턴스) : 데이터 저장시 인덱스를 직접 부여한다. 단, 인덱스를 건너뛰면
			에러가 발생하므로 주의해야 한다. 만약 중복된 인덱스가 있다면 삽입(끼워넣기)이 된다.
		 */
		list.add("소녀시대");
		list.add("빅뱅");
		list.add("트와이스");
		...
		/*
		방법2 : 확장 for 문의 경우 인덱스없이 전체 원소를 반복해 주므로
		...
		System.out.println(list.contains("블랙핑크") ? "블랙핑크 있음" : "블랙핑크 없음");//있음
		...
		/*
		위치를 모르는 경우에는indexOf()를 통해 해당 인스턴스의 인덱스를 찾은 후 삭제한다.
		 */
		...
		/*
		방법2 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는 삭제에 성공하면 true가 반환된다.
