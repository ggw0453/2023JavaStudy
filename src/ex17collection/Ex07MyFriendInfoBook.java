...
public class Ex07MyFriendInfoBook{ 
	...
	public static void main(String[] args) {
		...
		/* 컬렉션은 최초 생성시 크기를 지정하지 않는다. 입력 혹은 삭제되는 인스턴스
		에 따라 크기가 자동으로 설정된다. */
...
//		numOfFriends = 0;
	...
	public void addFriend(int choice) {
		...
		if(choice==1) {
			//고딩 친구를 선택한 경우 별명을 추가로 입력한다.
			System.out.print("별명:"); iNickname = scan.nextLine();
			HighFriend high = new HighFriend(iName, iPhone, iAddr, iNickname);
//			myFriends[numOfFriends++] = high;
			/* 기존 인스턴스베열에서는 인덱스를 위한 카운트가 필요했지만 컬렉션에서는
			필요하지 않다. 자동인덱싱이 지원되므로 add()를 통해 추가하기만 하면 된다 */
			lists.add(high);
		}
		else if(choice==2) {
...
//			}
	...
	public void searchInfo() {
		...
		while(itr.hasNext()) {
			...
			//if(searchName.equals(fr.name)==0) {
