...
//		}
		...
		/*
		if(조건식); => 세미콜론은 문장의 끝을 나타내므로 조건에 상관없이 중괄호에
		있는 문장이 실행된다. 따라서 사용에 주의해야한다.
		 */
		if(num%2 != 0);
		{
		...
		/*
		조건 검사후 실행할 문장이 1개인 경우에는 중괄호로 생랼할 수 있다.
		단, 실행문이 2개 이상이라면 한 문장만 if문에 포함되므로 반드시 중괄호를
		기술해야 한다.
		 */
		if(num%2 == 0)
			System.out.println("\n"+ num +"은 짝수이다");
		...
		/* 아스키코드를 알고 있을때의의 구현방법. 문자 0과 9가 아스키코드 48, 57에
		...
		/* 아스키코드를 모를 때 구연하는 방법으로 문자 '0'과 '9'는
		메모리에 저장시 아스키코드로 변화되므로 아래와같은 조건으로도 확인할 수 있다. */
