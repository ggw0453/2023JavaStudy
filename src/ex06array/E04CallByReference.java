...
/*
 ...
 : 메서드를 호출할때 참조값을 전달함으로써 서로 다른 지역에서도 동일한 메모리(Heap영역)를
 참조할 수 있게 하는 호출방식이다.
 메서드의 지역이 다르더라도 참조값을 통해 인스턴스를 참조하게되므로 A지역에서의 값이 변경이
...
public class E04CallByReference {
	...
	static void callByReference(int[] ref) {
		/*
	 	매개변수를 통해 전달받은 배열인스턴스의 참조값을 통해 0번 원소와 1번원소를 서로
	 	교환한다. 이때 배열에 저장된값이 서로 바뀌게된다.
		 */
		int temp;
		temp = ref[0];
		ref[0] = ref[1];
