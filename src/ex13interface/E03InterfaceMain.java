...
-추상클래스와 동일하게 상속을 목적으로 제작되므로 구현받은 하위클래스에서는 추상메서드를
반드시 오버라이딩 해야한다.
...
interface MyInterface1 {
	...
	/*
	멤버메서드 : 인터페이스에서 선언되는 메서드는 추상메서드로 선언된다.
...
class MyClass extends SimpleClass implements MyInterface1, MyInterface2 {
	...
	/* SimpleClass 클래스의 멤버메서드는 오버라이딩이 필수가 아니므로 필요한
	 경우에만 하면 된다. */
}
